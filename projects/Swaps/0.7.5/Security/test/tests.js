...
describe('Contract', function () {
    ...
    describe('depositing', () => {
        ...
        describe('unauthorized account converting from dai', () => {
            it('should revert', async () => {
                const signer1 = await ethers.provider.getSigner(1);
                let ex;
                try {
                    await contract.connect(signer1).fromDai(wethAddress, depositAmount);
                }
                catch(_ex) {
                    ex = _ex;
                }
        ...
        describe('authorized account converting from dai', () => {
            before(async () => {
            ...
            describe('unauthorized account converting back to dai', () => {
                it('should revert', async () => {
                    const signer1 = await ethers.provider.getSigner(1);
                    const balance = await weth.balanceOf(contract.address);
                    let ex;
                    try {
                        await contract.connect(signer1).toDai(wethAddress, balance);
                    }
                    catch (_ex) {
                        ex = _ex;
                    }
            ...
            describe('authorized account converting back to dai', () => {
                let expected;
                before(async () => {
                    const balance = await weth.balanceOf(contract.address);
